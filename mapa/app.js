...
function addBubble(p, ref) {
  ...
  const x = m.e, y = (Number.isFinite(p.alt) ? p.alt - ref.alt : 0), z = -m.n;
...
function rebuildRoutes(indicesOnly) {
  ...
  for (const r of state.routes) {
    ...
    for (const id of r.pointIds) {
      ...
      if (!p) continue;
      const m = toMeters(p.lat, p.lng, ref);
      pts.push(new THREE.Vector3(m.e, (Number.isFinite(p.alt) ? p.alt - ref.alt : 0), -m.n));
    }
...
function applyPivot() {
  pivot.position.set(-smoothU.e, -smoothU.alt, smoothU.n);
...
function onSessionStart() {
  ...
  rebuildWorld();
  showView('ar');
  toast('Caminando: los globos quedan fijos en el mundo real');
}
...
renderer.setAnimationLoop(() => {
  if (!arOn) return;
  if (demoOn) updateTargetU();
  const k = Math.min(1, 0.12);
  smoothU.e += (targetU.e - smoothU.e) * k;
  smoothU.n += (targetU.n - smoothU.n) * k;
  smoothU.alt += (targetU.alt - smoothU.alt) * k;
  applyPivot();
...
$('btn-add-point').addEventListener('click', () => {
  const g = currentLatLngAlt();
  if (!g) return toast('Esperando señal GPS…');
  const p = { id: newId(), name: '', lat: g.lat, lng: g.lng, alt: g.alt, icon: state.prefs.lastIcon || '📍', color: state.prefs.lastColor || PALETTE[1], note: '' };
  state.points.push(p);
  editingPointId = p.id;
  save();
  rebuildWorld();
