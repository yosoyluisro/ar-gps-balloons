...
near(haversineMeters(0, 0, 0.01, 0), 1111.949, 3, '0.01° de latitud en ecuador ≈ 1111.95 m');
near(haversineMeters(10, 10, 10.001, 10), 111.195, 0.5, '0.001° de latitud ≈ 111.19 m');
near(haversineMeters(40, -3.7, 40.001, -3.7), 111.195, 0.5, 'latitud España ≈ 111.19 m');

console.log('geo.test — deltaMeters');
let d = deltaMeters(10, 10, 10.001, 10);
near(d.north, 111.195, 0.5, 'delta norte 0.001° ≈ 111.19 m');
...
near(d.north, 11.12, 0.2, 'delta norte 0.0001° ≈ 11.12 m');

console.log('geo.test — metersToDelta (inversa)');
let p = metersToDelta(10, 10, 0, 111.195);
near(p.lat, 10.001, 0.00001, 'norte 111.195 m → +0.001° lat');
near(p.lng, 10, 0.00001, 'lng sin cambio');
p = metersToDelta(0, 0, 111.195, 0);
near(p.lng, 0.001, 0.00001, 'este 111.195 m en ecuador → +0.001° lng');
near(p.lat, 0, 0.00001, 'lat sin cambio');
p = metersToDelta(40.42, -3.7, 84.79, 0);
near(p.lng, -3.699, 0.00005, 'este 84.79 m a lat 40.42 → lng +0.001°');
...
// Londres→París: referencia ~343.5 km
const londonParis = haversineMeters(51.5074, -0.1278, 48.8566, 2.3522);
near(londonParis, 343500, 4000, 'Londres→París ≈ 343.5 km');
